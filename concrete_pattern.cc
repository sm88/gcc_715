...
void ConcretePattern::createPattern (){
    ...
    if (type != regCons) {
        //cerr << "Operands" << operandsIn.size() << endl;
        vector<Operand> newVector(operandsIn.begin(), operandsIn.end());
        tree->traverseTree (&newVector);
        //cerr << "NewVector" << newVector.size() << endl;
//        for(int l = 0; l < newVector.size(); l++){
//            std::cerr << newVector[l].getPredicate() << endl;
//            std::cerr << newVector[l].getMode() << endl;
//            std::cerr << newVector[l].getConstraint() << endl;
//        }
        //delete newVector;
    }
...
void ConcretePattern::trav(Node *aRoot){
    if(aRoot->isLeafNode()){
        return;
    }
    vector<Node*> vec = aRoot->getChildren();
